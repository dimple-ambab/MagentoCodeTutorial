...
Checkout.prototype = {
    initialize: function(accordion, urls){
        ...
        this.steps = ['login', 'billing', 'shipping', 'shipping_method', 'billing_comment','payment', 'review'];
...
//        //this.accordion.openNextSection(true);
    ...
    setDeliveryDate: function() {
        this.gotoSection('deliverydate', true);
    },
    setPayment: function() {
        //this.nextStep();
...
var BillingCommentMethod = Class.create();
BillingCommentMethod.prototype = {
    initialize: function(form, saveUrl){
            this.form = form;
            if ($(this.form)) {
                $(this.form).observe('submit', function(event){this.save();Event.stop(event);}.bind(this));
    ...
    save: function() {
        if (checkout.loadWaiting!=false) return;
        if (this.validate()) {
            checkout.setLoadWaiting('billing_comment');
            var request = new Ajax.Request(
                this.saveUrl,
                {
                    method:'post',
